...
public final class DateUtils {
    ...
    public static LocalDate getPropertyDate(Integer inputDate) {
        ...
        int year = (int) ((inputDate - inputDate % 10000) / 10000);
        int month = (int) ((inputDate - year * 10000 - day) / 100);
        return LocalDate.of(year, month, day);
    }

    public static OffsetDateTime getPropertyDateTime(Integer inputDate, Integer inputTime) {
        if (99999999 == inputDate) {
            return OffsetDateTime.of(2999, 01, 01, 00, 00, 00, 0, ZoneOffset.ofHours(8));
        }
        if (inputDate < 1 || inputTime < 1) {
            return OffsetDateTime.of(0000, 01, 01, 00, 00, 00, 0, ZoneOffset.ofHours(8));
        }
        int day = inputDate % 100;
        int year = 2000 + (int) ((inputDate - inputDate % 10000) / 10000);
        int month = (int) ((inputDate - (year - 2000) * 10000 - day) / 100);
        int second = inputTime % 100;
        int hour = (int) ((inputTime - inputTime % 10000) / 10000);
        int minute = (int) ((inputTime - hour * 10000 - second) / 100);
        return OffsetDateTime.of(year, month, day, hour, minute, second, 0, ZoneOffset.ofHours(8));
    }

    public static Date parseDate(String dateStr, String[] parttens) {
        for (String partten : parttens) {
            Date date = parseDate(dateStr, partten);
            if (date != null) {
                return date;
            }
        }
    ...
    public static String formatDateBetweenString(Date beginDate, Date endDate) {
        ...
        if (MyStringUtils.isNoneBlank(begin, end)) {
            return MyStringUtils.convertNullToString(begin) + " ~ " + MyStringUtils.convertNullToString(end);
        }
        return null;
    }
    ...
    public static Date[] parseDateBetweenString(String dateString) {
        Date beginDate = null;
        Date endDate = null;
        if (MyStringUtils.isNotBlank(dateString)) {
            String[] ss = MyStringUtils.split(dateString, "~");
            if (ss != null && ss.length == 2) {
                String begin = MyStringUtils.trim(ss[0]);
                String end = MyStringUtils.trim(ss[1]);
                if (MyStringUtils.isNoneBlank(begin, end)) {

...
public class FileUtils extends org.apache.commons.io.FileUtils {
    ...
    public static void writeToFile(String fileName, String content, boolean append) {
        try {
            FileUtils.write(new File(fileName), content, FILE_CHARSET, append);
    ...
    public static String downFile(File file, HttpServletRequest request, HttpServletResponse response, String fileName) {
        ...
        try {
            ...
            if ("MSIE".contains(userAgent) || "Trident".contains(userAgent)) {
                // IE内核浏览器时的文件名处理
                fileName = Encodes.urlEncode(StringUtils.isBlank(fileName) ? file.getName() : fileName);
            } else {
                // 非IE浏览器时的文件名处理
                fileName = new String(fileName.getBytes(Constants.CHARSET_UTF8), "ISO-8859-1");
            }
//            response.addHeader("Content-Disposition", "attachment; filename=\"" +
//                    Encodes.urlEncode(MyStringUtils.isBlank(fileName) ? file.getName() : fileName) + "\"");
    ...
    public static Map<String,Object> upload(
            MultipartFile file, String uploadFilePath, String childPath, HttpServletRequest request) {
        Map<String,Object> jsonObject = new HashMap<>();
        String destDir = uploadFilePath + childPath;
        try {
            if (file.isEmpty()) {
            ...
            //创建绝对路径
            String realPath = destDir + "/" + dateFormat;
            ...
            String url = childPath + "/" + dateFormat + "/" + fileEnd + suffix;
            //上传
            file.transferTo(uploadFile);
            jsonObject.put("success", true);
            jsonObject.put("url", url);
            jsonObject.put("fileName", fileEnd + suffix);
            jsonObject.put("realPath", uploadFile.getPath());
        } catch (Exception e) {
     ...
     * 传入文件路径，将文件转换为字节
     **/
    public static byte[] fileToBinary(String filePath) throws Exception {
        InputStream in = null;
        ByteArrayOutputStream out = null;
        try {
            in = new BufferedInputStream(new FileInputStream(filePath));
            return fileToBinary(in);
        } catch (Exception e) {
            throw e;
        } finally {
            if (in != null) {
     ...
     * 把输入流转换为字节
     **/
    public static byte[] fileToBinary(InputStream in) throws Exception {
     ...
     * @throws Exception
     */
    public static byte[] zipFiles(File[] files, String encoding) throws Exception {
        ByteArrayOutputStream out = null;
            ...
            for (int i = 0; i < files.length; i++) {
                ...
                if (file.exists()) {
                    ...
                    try {
                        ...
                        log.error(file.getName() + "文件压缩出错！", ex);
                    } finally {
                        if (fin != null) {
                            fin.close();
                        }
     ...
     * @param response
     */
    public static void respFileHeaderSet(HttpServletResponse response, String fileName, byte[] fileByte) {
        try {
            response.reset();
    ...
    public static String getFileRootPath() {
        ...
        return systemConfig.getFileRoot();
    }

    /*
     * <li>创建人：mayonghui
     * <li>创建时间：2019/1/21 15:48
     * <li>创建目的：【根据路径获取文件，对文件获取进行统一的管理】
     * <li>修改目的：【修改人：修改目的，修改时间】
     * <li>参数列表：[path:绝对路径或者相对路径。相对路径前面不能有文件分隔符]
     * <li>返回值类型：java.io.File
     **/
    public static File getFile(String path) {
        String fileRoot = getFileRootPath();
        if (StringUtils.isBlank(path)) {
            if (log.isDebugEnabled()) {
                log.debug("文件路径不能为null。");
            }
            return null;
        }
    ...
    public static String getFilePath(String path) {
        return path + DateUtils.format(new Date(), "yyyyMMdd");
    }
    ...
    public static void writeData(File file, byte data[]) {
        try {
            ...
            try {
                outt = new RandomAccessFile(file, "rw");
                in = ByteBuffer.wrap(data);
                channel = outt.getChannel();
                channel.write(in);
            } finally {
                in = null;
                if (channel != null) {
